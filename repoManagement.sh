...
read projectName
curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X POST "http://172.17.0.2/api/v4/projects?name="$projectName""
...
curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X GET "http://172.17.0.2/api/v4/projects/" | jq '.'
...
read projectId
curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X DELETE "http://172.17.0.2/api/v4/projects/$projectId"
...
read projectId
curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X GET "http://172.17.0.2/api/v4/projects/$projectId/members" | jq '.'
...
case "$response" in
    ...
    while read -u4 t; do
        echo $t
        curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X POST "http://172.17.0.2/api/v4/projects?name="$t""
...
case "$response" in
    ...
    while read -u4 t; do
        echo $t
        curl --header "PRIVATE-TOKEN: NwDXM3z-8bVG5G5byhkN" -X DELETE "http://172.17.0.2/api/v4/projects/$t"
